...
int main()
{
    ...
    vector<ll> p, q;
    rep(i, 0, N / 2) p.push_back(A[i]);
    rep(i, N / 2, N) q.push_back(A[i]);

    vector<ll> p_sum = sums(p);
    vector<ll> q_sum = sums(q);
    sort(all(p_sum));
    sort(all(q_sum));

    rep(i, 0, p_sum.size())
    {
        int pos = lower_bound(all(q_sum), K - p_sum[i]) - q_sum.begin();
        if (pos < q_sum.size() && q_sum[pos] == K - p_sum[i])
        {
            cout << "Yes" << endl;
            return 0;
        }
