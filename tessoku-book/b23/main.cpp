...
int main()
{
    ...
    vector<vector<double>> dp((1 << 16), vector<double>(20, INF));
    dp[0][0] = 0;
    rep(i, 0, (1 << N))
    {
        rep(j, 0, N)
        {
            ...
            rep(k, 0, N)
            {
                ...
                double d = sqrt((X[j] - X[k]) * (X[j] - X[k]) * 1.0 + (Y[j] - Y[k]) * (Y[j] - Y[k]) * 1.0);
                dp[(1 << k) + i][k] = min(dp[(1 << k) + i][k], dp[i][j] + d);
            }
        }
    }

    cout << fix(6) << dp[(1 << N) - 1][0] << endl;
}