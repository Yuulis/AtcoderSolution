...
int main()
{
    ...
    vector<int> maxFromL(N + 1);
    maxFromL[1] = A[1];
    rep(i, 2, N + 1) maxFromL[i] = max(maxFromL[i - 1], A[i]);

    vector<int> maxFromR(N + 1);
    maxFromR[N] = A[N];
    rrep(i, N - 1, 1) maxFromR[i] = max(maxFromR[i + 1], A[i]);
    ...
    rep(i, 0, D)
    {
        ...
        cout << max(maxFromL[L - 1], maxFromR[R + 1]) << endl;
    }
}