...
void dfs(vector<string> &grid, vector<vector<bool>> &visited, const int &H, const int &W, const int &K, int x, int y, int steps, int &ans)
{
    if (steps == K)
    {
        ans++;
    ...
    rep(dir, 0, 4)
    {
        ...
        {
            dfs(grid, visited, H, W, K, nx, ny, steps + 1, ans);
        }
    }
...
int main()
{
    ...
    {
        if (grid[i][j] == '.')
            dfs(grid, visited, H, W, K, i, j, 0, ans);
    }
