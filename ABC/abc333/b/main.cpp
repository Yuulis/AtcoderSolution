...
int main()
{
    ...
    rep(i, 0, 5) rep(j, 0, 5)
    {
        ...
        {
            shorts.push_back(vertexes[i] + vertexes[j]);
        }
        else
        {
            longs.push_back(vertexes[i] + vertexes[j]);
        }
    }

    if (count(all(shorts), S) && count(all(shorts), T))
    {
        cout << "Yes" << endl;
    }
    else if (count(all(longs), S) && count(all(longs), T))
    {
        cout << "Yes" << endl;
    }
    else
