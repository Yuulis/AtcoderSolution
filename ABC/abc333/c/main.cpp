...
int main()
{
    ...
    {
        ll x = 0;
        rep(j, 0, i) x += pow(10, j);
        repunits.push_back(x);
    }

    int l = repunits.size();
    vector<ll> trio;
    rep(i, 0, l) rep(j, 0, l) rep(k, 0, l) trio.push_back(repunits[i] + repunits[j] + repunits[k]);

    sort(all(trio));
    trio.erase(unique(all(trio)), trio.end());
