...
int main()
{
    ...
    rep(i, 0, H) rep(j, 0, W) rows[i].insert(j), cols[j].insert(i);
    ...
    while (Q--)
    {
        ...
        auto itr = cols[C].lower_bound(R);
        if (itr != cols[C].begin()) {
            rows[*prev(itr)].erase(C);
            cols[C].erase(prev(itr));
        ...
        itr = rows[R].lower_bound(C);
        if (itr != rows[R].begin()) {
            cols[*prev(itr)].erase(R);
            rows[R].erase(prev(itr));
